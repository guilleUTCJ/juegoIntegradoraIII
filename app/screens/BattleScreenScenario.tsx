...
export default function BattleScreenScenario({ route, navigation }: any) {
    // ✅ Protección contra route.params undefined
    // Acepta roomId directamente O room.id por si viene el objeto completo
    ...
    useEffect(() => {
        if (!roomId) {
            console.error('BattleScreenScenario: roomId es undefined. Params recibidos:', JSON.stringify(route?.params));
            return;
        }
    ...
    const handleAttack = useCallback(() => {
        ...
        setActions(prev => ({ ...prev, [key]: { ...prev[key], attack: true } }));
        setTimeout(() => {
            setActions(prev => ({ ...prev, [key]: { ...prev[key], attack: false } }));
        }, 200);
        Animated.sequence([
    ...
    const handleBlock = (value: boolean) => {
        const key = isP1 ? 'p1' : 'p2';
        setActions(prev => ({ ...prev, [key]: { ...prev[key], block: value } }));
    };
    ...
    // ✅ Pantalla de error si no llegó roomId
    if (!roomId) {
        return (
            <View style={styles.centered}>
                <Text style={styles.loadingText}>❌ Error: no se recibió roomId</Text>
        ...
        <View style={styles.container}>
            ...
            <View style={styles.hudContainer}>
                <View style={styles.playerInfo}>
                    <Text style={styles.playerName}>{meSafe?.email?.split('@')[0] ?? '...'}</Text>
                ...
                <View style={[styles.playerInfo, { alignItems: 'flex-end' }]}>
                    <Text style={styles.playerName}>{enemySafe?.email?.split('@')[0] ?? '...'}</Text>
                    <View style={styles.hpBarBg}>
            ...
            <View style={styles.centerStage}>
                <Animated.Text style={[styles.petSprite, { left: 100, transform: [{ translateX: isP1 ? attackTranslate : 0 }] }]}>🐶</Animated.Text>
                <Animated.Text style={[styles.petSprite, { right: 100, transform: [{ translateX: !isP1 ? attackTranslate : 0 }] }]}>🐺</Animated.Text>
            </View>

            <View style={styles.controlsOverlay}>
                <View style={styles.joystickWrapper} {...panResponder.panHandlers}>
                    <View style={styles.joystickBase}>
                        <Animated.View style={[styles.joystickKnob, { transform: joystick.getTranslateTransform() }]} />
                    </View>
                </View>
                <View style={styles.actionButtons}>
                    ...
                    <TouchableOpacity style={[styles.btnCircle, styles.btnDefend]} onPressIn={() => handleBlock(true)} onPressOut={() => handleBlock(false)}>
...
const styles = StyleSheet.create({
    ...
    hudContainer: { position: 'absolute', top: 30, left: 30, right: 30, flexDirection: 'row', justifyContent: 'space-between', zIndex: 10 },
    playerInfo: { width: '40%' },
    ...
    controlsOverlay: { position: 'absolute', bottom: 30, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 50 },
    joystickWrapper: { width: 120, height: 120 },
    joystickBase: { width: 100, height: 100, borderRadius: 50, backgroundColor: 'rgba(255,255,255,0.15)', justifyContent: 'center', alignItems: 'center' },
    joystickKnob: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.8)' },
    actionButtons: { flexDirection: 'row', gap: 20, alignItems: 'center' },
    btnCircle: { width: 70, height: 70, borderRadius: 35, justifyContent: 'center', alignItems: 'center' },
